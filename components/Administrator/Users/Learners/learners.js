...
import { getS3FileUrl, getCognitoUsers } from '../../../../utilities/others'
...
function Learners() {
  ...
  // debugger
  const studentObj = useSelector((state) => state.StudentHeaderReducer)
  useEffect(() => {
    ;(async () => {
      try {
  ...
  useEffect(() => {
    if (studentObj?.students?.length === 0) {
      setStudents(dispatch)
    }
  }, [studentObj])
  useEffect(() => {
    let studentsData = studentObj.students.filter((a) => a.username)
    if (studentBookings.length > 0) {
      ...
      studentBookings.forEach((item) => {
        // debugger
        const found = studentsData.find((a) => a.username == item.emailId)
        if (found) {
          if (found.bookings && found.bookings.length) {
          } else {
            found.bookings = []
          }
          found.signupby = 'Internal'
          ...
          studentsData.push(newItem)
        }
      })
    }
    setStudentsFiltered(studentsData)
    setStudentBookingsFiltered(studentsData)
  }, [studentObj.students, studentBookings, cognitoUsers])
  ...
  const downloadTemplate = async () => {
    ...
    // let csv = convertArrayOfObjectsToCSV(studentsFiltered)
    // if (csv == null) return
  ...
  console.log('studentObj', studentObj?.students)
  ...
  function editRecord(username) {
    const rec = studentsFiltered.find((i) => i.username === username)
    dispatch({
      type: ACTION_KEYS.SET_STUDENT_PURCHASES,
      payload: {
        id: rec.id,
        data: rec.bookings,
      },
    })
    router.push(`/admin/users/learners/${rec.id}`)
  }
  ...
  useEffect(() => {
    ...
    if (filterBy && filterText) {
      const filtered = studentsFiltered.filter((item) => {
        switch (filterBy) {
          case 'name':
            return (
      ...
      setStudentBookingsFiltered(studentsFiltered)
    }
  }, [filterText, filterBy])
  useEffect(() => {
  ...
  return (
    <div className={classes.container}>
      ...
      <DataTable
        ...
        style={{width: '100%'}}
      />
      {showModal && <AddLearners show={showModal} setShow={setShowModal} />}
    </div>
  )
}
