...
const verifyEmailIdentity = async (recipientEmailAddress) => {
  const commandVerifyEmail = new VerifyEmailIdentityCommand({
    EmailAddress: recipientEmailAddress,
  })

  try {
    await sesClient.send(commandVerifyEmail)
...
export const sendEmail = async (toAddress, fromAddress, body, subject) => {
  ...
  try {
    ...
    debugger
    if (JSON.stringify(e).includes(fromAddress)) {
      try {
        await verifyEmailIdentity(fromAddress)
      } catch (er) {
        throw er
      }
    }
    if (JSON.stringify(e).includes(toAddress)) {
      try {
        await verifyEmailIdentity(toAddress)
      } catch (er1) {
        throw er1
      }
    }
    throw { message: 'Email not verified. Please try again after verification' }
  }
}
export const getS3FileUrl = async (
  image_name,
  bucketName = config.aws_user_files_s3_bucket,
  downloadFileName = '',
) => {
  try {
    const command = new GetObjectCommand(
      bucketName
        ...
        : {
            Bucket: config.aws_user_files_s3_bucket, // The name of the bucket. For example, 'sample_bucket_101'.
...
export const setS3FileUrl = async (
  image_name,
  file,
  previous_image_name = '',
) => {
  ...
  const ext = image_name.substr(image_name.lastIndexOf('.'))
  const params = {
    ContentType: `image/${dictMime[ext]}`,
